...
router.get('/tenants', async (req, res) => {
  try{
    ...
    const agreements = tenantIds.length ? await Agreement.findAll({ where: { tenantId: tenantIds } }) : [];
    const flatIds = agreements.map(a=>a.flatId).filter(Boolean);
    const flats = flatIds.length ? await Flat.findAll({ where: { id: flatIds } }) : [];
    const agByTenant = {};
    agreements.forEach(a=>{ agByTenant[a.tenantId] = a; });
    const flatsById = {};
    flats.forEach(f=>{ flatsById[f.id] = f; });
  ...
  }catch(e){ console.error('owner list tenants failed', e); res.status(500).json({ error: 'failed' }); }
});

// Create a tenant (owner can create tenants for their society)
router.post('/tenants', async (req, res) => {
  try{
    const { name, phone: rawPhone, address, rent, deposit, gender, move_in, move_out, flatId } = req.body;
    const phone = String(rawPhone || '').replace(/\D/g, '');
    if(!phone) return res.status(400).json({ error: 'phone required' });
    const existing = await User.findOne({ where: { phone } });
    if(existing) return res.status(400).json({ error: 'user exists' });
    const user = await User.create({ name: name||phone, phone, role: 'tenant', societyId: req.user.societyId, address, rent: rent || null, deposit: deposit || null, gender: gender || null, move_in: move_in || null, move_out: move_out || null });

    // If flatId provided, create an Agreement linking tenant to flat (owner is current user)
    if(flatId){
      try{ await Agreement.create({ flatId, ownerId: req.user.id, tenantId: user.id }); }catch(err){ console.warn('[owner/create] failed to create agreement', err && err.message); }
    }

    res.json({ user });
  }catch(e){ console.error('owner create tenant failed', e); res.status(500).json({ error: 'failed', detail: e && e.message }); }
});
...
router.put('/tenants/:id', async (req, res) => {
  try{
    ...
    if(!tenant || tenant.societyId !== req.user.societyId) return res.status(404).json({ error: 'not found' });
    const allowed = ['name','address','gender','move_in','move_out','rent','deposit','phone','status'];
    const updates = {};
    ...
    if(flatId){
      ...
      }catch(err){ console.warn('[owner/update] failed to create agreement', err && err.message); }
    }

    res.json({ user: tenant });
  }catch(e){ console.error('owner update tenant failed', e); res.status(500).json({ error: 'failed', detail: e && e.message }); }
});
...
router.post('/tenants/:id/status', async (req, res) => {
  try{
    ...
    try{ console.log('[owner/status] req.user.id=', req.user && req.user.id, 'req.user.societyId=', req.user && req.user.societyId, 'targetTenantId=', id); }catch(e){}
    try{ console.log('[owner/status] tenant found=', !!tenant, 'tenant.societyId=', tenant && tenant.societyId); }catch(e){}
    ...
    if(!tenant.societyId){
      try{
        ...
        console.log('[owner/status] assigned missing societyId to tenant', tenant.id, '->', req.user.societyId);
      }catch(e){
        console.warn('[owner/status] failed to assign societyId to tenant', tenant.id, e && e.message);
      }
    ...
    res.json({ user: tenant });
  }catch(e){ console.error('owner tenant status failed', e); res.status(500).json({ error: 'failed', detail: e && e.message }); }
});
...
router.get('/maintenance', async (req, res) => {
  try{
    ...
    const complaints = await Complaint.findAll({ where: { societyId: req.user.societyId, raised_by: req.user.id } });
    res.json({ maintenance: complaints });
  }catch(e){ console.error('owner maintenance list failed', e); res.status(500).json({ error: 'failed' }); }
});
...
router.post('/maintenance', async (req, res) => {
  ...
  }catch(e){ console.error('owner create maintenance failed', e); res.status(500).json({ error: 'failed', detail: e && e.message }); }
});
...
router.post('/upload', async (req, res) => {
  ...
  }catch(e){ console.error('owner upload failed', e); return res.status(500).json({ error: 'upload failed', detail: e && e.message }); }
});

// Documents uploaded by this owner
router.get('/documents', async (req, res) => {
  try{
    const docs = await Document.findAll({ where: { societyId: req.user.societyId, uploaded_by: req.user.id } });
    res.json({ documents: docs });
  }catch(e){ console.error('owner docs list failed', e); res.status(500).json({ error: 'failed' }); }
});
...
router.post('/documents', async (req, res) => {
  try{
    ...
    const doc = await Document.create({ title, file_url, file_type, uploaded_by: req.user.id, societyId: req.user.societyId });
    res.json({ document: doc });
  }catch(e){ console.error('owner create doc failed', e); res.status(500).json({ error: 'failed', detail: e && e.message }); }
});
