...
router.post('/societies', async (req, res) => {
  ...
  const society = await Society.create({ name, country, city, area, created_by: req.user.id });
  res.json({ society });
});

router.put('/societies/:id', async (req, res) => {
  const { id } = req.params;
  const { name, country, city, area, mobile_number, status, subscription_start_date, subscription_end_date } = req.body;
  const soc = await Society.findByPk(id);
  if(!soc) return res.status(404).json({ error: 'not found' });
  await soc.update({ name, country, city, area, mobile_number, status, subscription_start_date, subscription_end_date });
...
router.post('/admins', async (req, res) => {
  ...
  if(!phone || !password || !societyId) return res.status(400).json({ error: 'phone,password,societyId required' });
...
router.put('/admins/:id', async (req, res) => {
  ...
  if(typeof societyId !== 'undefined'){
    ...
    const ids = Array.isArray(societyId) ? societyId : [societyId];
    for(const sid of ids){
      try{ await models.AdminSociety.create({ userId: id, societyId: sid }); }catch(e){ }
    }
  }
