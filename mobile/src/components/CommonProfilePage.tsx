...
type Props = {
  ...
  title?: string; // Custom title for the profile page
  refetch?: boolean; // Trigger to refetch data from backend
};
...
export default function CommonProfilePage({
  ...
  // Refetch profile data when refetch prop changes
  useEffect(() => {
    if (refetch && (fetchEndpoint || user)) {
      if (fetchEndpoint) {
        fetchProfile();
      } else {
        // If no custom endpoint, refetch user data from default endpoint
        refetchUserData();
      }
    }
  }, [refetch]);
